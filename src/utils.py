...
def find_parameter(value, param_results, param):
    ...
    if value.lower() not in param_results:
        print(str(((param.split(".")[-1]).replace("_", " ")).title()) + ": " + str(
            value) + " was not found. Check available parameter: \n")
...
def print_json(json_obj):
    ...
    """
    print(json.dumps(json_obj, indent=2, default=str)
          )  # Display JSON with its correct identation
